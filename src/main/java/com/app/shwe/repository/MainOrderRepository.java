...
public interface MainOrderRepository extends JpaRepository<MainOrder, Integer> {
	...
	@Query(value = "SELECT mo.order_id, mo.id, mo.sys_key, mo.created_date, u.user_name, mo.status "
			+ "FROM main_order mo " + "JOIN user u ON mo.user_id = u.id "
			+ "WHERE (mo.sys_key LIKE %:searchString% OR u.user_name LIKE %:searchString%) AND mo.status = :status "
			+ "ORDER BY mo.created_date DESC", countQuery = "SELECT COUNT(*) FROM main_order mo JOIN user u ON mo.user_id = u.id "
					+ "WHERE (mo.sys_key LIKE %:searchString% OR u.user_name LIKE %:searchString%) AND mo.status = :status", nativeQuery = true)
	Page<Object[]> findBySysKeyAndUserName(@Param("searchString") String sysKey, @Param("status") String status,
			Pageable pageable);

...
public interface EmbassyLetterRepository extends JpaRepository<EmbassyLetter, Integer>{
	...
	@Query("SELECT new com.app.shwe.dto.EmbassyLetterProjectionDTO(v.id,v.syskey,v.passportBio, v.visaPage, v.contactNumber,v.user.userName,v.status) FROM VisaExtension v WHERE v.user.id = :id ORDER BY v.id DESC")
